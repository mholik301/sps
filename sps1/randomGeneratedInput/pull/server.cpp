...
static void* gstreamer_server(void* notused)
{
    ...
    gst_rtsp_media_factory_set_launch (factory,
                                       "( appsrc name=mysrc ! videoconvert ! video/x-raw,format=(string)I420 ! x264enc ! rtph264pay name=pay0 pt=96 )");
...
IplImage* random_image(int width, int height)
{
...
#endif
#ifdef GEN_COLOR
                    img->imageData[y * img->widthStep + x * 3 + c] = incr;
...
main (int argc, char *argv[])
{
    ...
    // work besides the server
    for(int i=0; i<(MAIN_ITERATION_LEN*FPS); i++)
    {
        std::cout << "main iteration " << i << "/" << (MAIN_ITERATION_LEN*FPS) << " (" << i/FPS << "/" << MAIN_ITERATION_LEN << " s)" << std::endl;

        IplImage* iplImg = random_image(WIDTH, HEIGHT);

        // push image to buffer
        g_async_queue_push(queue, iplImg);
